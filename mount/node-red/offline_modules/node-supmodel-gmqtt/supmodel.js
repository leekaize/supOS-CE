...
module.exports = function (RED) {
    ...
    function SelectModel(config) {
        ...
        switch (node.protocol) {
            case "opcua": node.bridge = opcua.newOpcuaBridge(node, node.mappings, node.interval); break;
        ...
        node.on('input', function (msg) {
            var errorMsg = node.bridge.receive(msg, node.mappings);
            if (errorMsg) {
        ...
        // 监听节点删除事件
        this.on('delete', function (done) {
            node.context().global.set(node.id, null);
            done();
        });
